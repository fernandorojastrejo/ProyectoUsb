$('.iboxProducto').children('.ibox-content').addClass('sk-loading');
...
var catId = $("#CategoriaId").val();
$("#ordenamientoProducto").on("change", function () {

    if (!buscar) {
        if (!nuevosProductos) {
            obtenerProductosCategoria(catId, 1);
            $(".pagination").pagination('drawPage', 1);
        } else {
            obtenerNuevosProductos();
        }        
    } else {
        clickBtnBuscar(catId, 1);
    }
    
    //dibujarPaginador(catId);
});
...
var nuevosProductos = false;
function obtenerNuevosProductos() {
    nuevosProductos = true;
    $('.iboxProducto').children('.ibox-content').addClass('sk-loading');
    orderBy = $("#ordenamientoProducto").val();
    var urlPeticion = apiUrl + "api/producto/ObtenerNuevosProductos/" + orderBy;
...
function obtenerProductosCategoria(categoriaId, pagina) {
    ...
    orderBy = $("#ordenamientoProducto").val();
    var urlPeticion = apiUrl + "api/producto/ObtenerProductosCategoriaPaginado/" + categoriaId + "/" + pagina + "/" + orderBy;
...
function buscarProductosCategoria(categoriaId, pagina) {
    ...
    //var urlPeticion = apiUrl + "api/producto/ObtenerProductosCategoria/" + categoriaId;
    orderBy = $("#ordenamientoProducto").val();
    var urlPeticion = apiUrl + "api/producto/BuscarProductosCategoria/" + categoriaId + "/" + descripcion + "/" + color + "/" + capacidad + "/" + material + "/" + pagina + "/" + orderBy;
    ajaxProducto(urlPeticion, true);
}
...
function ajaxProducto(urlPeticion, paginado) {
    ...
    $.ajax({
        ...
        if (data.length > 0) {
            $.each(data, function (index, item) {
                ...
                if (item.ProductoFotografia.length > 0) {
                    html = ''
                        ...
                        //+ '<a href="DetalleProducto/' + item.Codigo + '" class="btn btn-xs btn-outline btn-primary" data-id="' + item.Codigo + '">Detalle <i class="fa fa-long-arrow-right"></i> </a>'
                        + '<a href="DetalleCodigo/' + item.Codigo + '" class="btn btn-xs btn-outline btn-primary" data-id="' + item.Codigo + '">Detalle <i class="fa fa-long-arrow-right"></i> </a>'
                        + '</div>'
                        + '</div>'
                        + '</div>'
                        + '</div >'
            ...
            //$('.iboxProducto').children('.ibox-content').removeClass('sk-loading');
...
function pagination(categoriaId, totalCount) {
    ...
    $('.pagination').pagination({
        ...
        onPageClick: function (pageNumber) {
            buscar == false ? urlPeticion = apiUrl + "api/producto/ObtenerProductosCategoriaPaginado/" + categoriaId + "/" + pageNumber + "/" + orderBy : urlPeticion = apiUrl + "api/producto/BuscarProductosCategoria/" + categoriaId + "/" + descripcion + "/" + color + "/" + capacidad + "/" + material + "/" + pagina + "/" + orderBy;
            ajaxProducto(urlPeticion, true);
            //goTopProducto();
        }
    });
